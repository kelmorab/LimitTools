...
if prefit and not errors:
  print "cat\t process\t prefit\t s+b\t b-only\t s+b/pre\t b/pre"
...
while True:
    ...
    if prefit and norm_p and errors:
        ...
        sboverpre=0
        if norm_p.getVal()!=0:
	  boverpre=norm_b.getVal()/norm_p.getVal()
	  sboverpre=norm_s.getVal()/norm_p.getVal()
	else:
	  boverpre=1.0
	  sboverpre=1.0 
        print "%-30s %-30s %7.3f %7.3f  %7.3f %7.3f %7.3f" % (m.group(1), m.group(2), norm_p.getVal(), norm_s.getVal(), norm_b.getVal(), sboverpre, boverpre)
    else:
        if errors:
