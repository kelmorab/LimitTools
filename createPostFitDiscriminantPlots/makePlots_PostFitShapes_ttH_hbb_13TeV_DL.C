 ...
 /////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  for( int iFit=0; iFit<Nfits; iFit++ ){
    ...
    for( int iSample=0; iSample<Nsamples; iSample++ ){
      ...
      hist_sl_cat[iSample] = new TH1D(hist_name_sl, ";;Number of events per category", 8, 0, 8 );
      hist_dl_cat[iSample] = new TH1D(hist_name_dl, ";;Number of events per category", 5, 0, 5 );
    }
	  
    for( int iCat=0; iCat<Ncategories; iCat++ ){

//       const bool useArray = ( categories.at(iCat) == "sl_jge6_t3_high" );
      const bool useArray = false;
	...
	if( categories[iCat].BeginsWith("sl_") ){
	  hist_sl_cat[iSample]->SetBinContent(iCat+1, sum);
	  hist_sl_cat[iSample]->SetBinError(iCat+1, sum_err);
	}
	else{
...
//     }
    ...
    std::cout<<"n bins SL "<<Nbins_sl<<std::endl;
    ...
    std::cout<<"creating ratio"<<std::endl;
    ...
    for( int iBin=0; iBin<Nbins_sl; iBin++ ){
      myRatio_sl->GetXaxis()->SetBinLabel(iBin+1,category_names[iBin]);
    }
